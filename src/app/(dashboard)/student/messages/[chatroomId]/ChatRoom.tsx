...
export default function ChatRoom({ chatroomId }: { chatroomId: string }) {
  ...
  // lastMsgIdRef is updated by the scroll effect (runs after this one) — so on any given
  // messages.length change this effect sees the PREVIOUS last-msg id, which is exactly what
  // we need: if the id didn't change it was a load-more prepend, not a new append.
  useEffect(() => {
    ...
    // Load-more prepend — last message unchanged, don't increment
  ...
  // Tracks the last message's identity so both effects can tell append from prepend.
  // Updated by the scroll effect (runs second); read by the anchor effect (runs first).
  ...
  useEffect(() => {
    ...
    document.title =
      chatroomUnread > 0
        ? `iExcelo - Student | ${partnerName} (${chatroomUnread})`
        : `iExcelo - Student | ${partnerName}`;
    return () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Clear messages + anchor on leave — prevents cached messages from firing
      // anchor effect before fetchMessages sets the real anchor on next visit
      setAnchor(null);
  ...
  // Fetch partner presence once partner is known (may be after chatrooms load)
  useEffect(() => {
  ...
  // Also owns lastMsgIdRef — always updates it so the anchor effect (which runs first)
  // can distinguish prepend (load-more) from append (new message) on the next cycle.
  useEffect(() => {
    if (!messages.length) return;
    const lastMsg = messages[messages.length - 1];
    const lastMsgId = lastMsg?.tempId ?? lastMsg?.id;
    const isAppend = lastMsgId !== lastMsgIdRef.current;
    lastMsgIdRef.current = lastMsgId; // always keep current
    ...
    // Load-more prepend — don't scroll or increment badge
    if (!isAppend) return;
